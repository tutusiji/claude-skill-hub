> ⚠️ **本文档已过时**：描述的 `web` + `git-server` 双容器拓扑中，`git-server` 容器已在 `f8aa05b` 架构重构中移除；marketplace URL 也已统一为 `http://your-internal-host:7789/skill-hub.git`（去掉 `/git/`）。当前为单 `web` 容器 + 宿主机 git marketplace。最新部署指南见 [`offline-deploy.md`](./offline-deploy.md)。
...
```bash
git remote add internal http://your-internal-host:7789/claude-skill-hub.git
...
```bash
# Web UI
curl -sS http://your-internal-host:7788 | grep '插件市场'

# Git 仓库
git clone http://your-internal-host:7789/claude-skill-hub.git /tmp/verify
...
```bash
claude plugin marketplace add http://your-internal-host:7789/claude-skill-hub.git
...
```bash
claude plugin marketplace add http://your-internal-host:8081/git/claude-skill-hub.git
```
...
# 测试 git 仓库是否响应
curl "http://your-internal-host:7789/claude-skill-hub.git/info/refs?service=git-upload-pack"
