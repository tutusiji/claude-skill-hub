...
// 查找插件根目录：优先找 .claude-plugin/plugin.json，回退找 skills/ 目录（纯 skill 集合包），再回退找单 skill 平铺包
// 递归搜索，支持 ZIP 内任意深度的嵌套目录（如 skills-main/skills-main/...）
export type PluginRootType = 'plugin' | 'skills' | 'skill';
export type PluginRoot = { path: string; type: PluginRootType };
...
// 检查是否为单 skill 平铺包：根目录直接有 SKILL.md（带 frontmatter），没有 .claude-plugin 也没有 skills/
function isSingleSkillDir(dir: string): boolean {
...
export function findPluginRoot(dir: string): PluginRoot | null {
  // 优先级 1：检查当前目录是否有 .claude-plugin/plugin.json（标准插件）
  if (existsSync(join(dir, '.claude-plugin', 'plugin.json'))) {
    return { path: dir, type: 'plugin' };
  }
  // 优先级 2：检查当前目录是否有 skills/ + SKILL.md（纯 skill 集合包）
  if (hasSkillsDir(dir)) {
    return { path: dir, type: 'skills' };
  }
  // 优先级 3：检查当前目录是否直接有 SKILL.md（单 skill 平铺包）
  if (isSingleSkillDir(dir)) {
    return { path: dir, type: 'skill' };
  }
  // 递归检查所有子目录
...
export function validatePluginDir(pluginPath: string, rootType: PluginRootType = 'plugin'): ValidationResult {
  const errors: string[] = [];
  ...
  if (rootType === 'skill') {
    // 单 skill 平铺包：根目录直接有 SKILL.md，没有 plugin.json 和 skills/
    ...
    let skillName = cleanName;
    let skillDescription = `Skill: ${cleanName}`;

    try {
      const content = readFileSync(skillMdPath, 'utf-8');
      if (!content.startsWith('---')) {
        errors.push('SKILL.md 缺少 YAML frontmatter（需以 --- 开头）');
      } else {
        const fm = parseFrontmatter(content);
        if (fm.name) {
          skillName = fm.name;
        } else {
          warnings.push('SKILL.md frontmatter 缺少 name 字段，将使用目录名');
        }
        if (fm.description) {
          skillDescription = fm.description;
        } else {
          warnings.push('SKILL.md frontmatter 缺少 description 字段');
        }
      }
    } catch (e) {
      errors.push(`SKILL.md 读取失败: ${e instanceof Error ? e.message : String(e)}`);
      return { passed: false, errors, warnings, summary };
    }

    manifest = { name: cleanName, version: '1.0.0', description: skillDescription };
    summary.skillsCount = 1;
    warnings.push('此包为单 skill 平铺包（无 .claude-plugin/plugin.json、无 skills/ 目录），已自动生成插件元数据');

  } else if (rootType === 'skills') {
    // 纯 skill 集合包：没有 plugin.json，从目录名自动生成元数据
    const dirName = basename(pluginPath);
    // 剥离常见后缀：-main, -master, -6.1.0 等版本号
    const cleanName = dirName.replace(/-(main|master)$/, '').replace(/[-.]\d+\.\d+\.\d+$/, '');
    // 尝试从 README.md 提取描述
  ...
  // 4. 检查 skills 目录（对单 skill 包跳过，因为已经检查过）
  if (rootType !== 'skill') {
...
export function validateUploadedFile(filepath: string): ValidationResult {
  ...
  try {
    ...
    // 查找插件根目录（支持 plugin 包、纯 skill 集合包、单 skill 平铺包）
    const pluginRoot = findPluginRoot(tempDir);
    if (!pluginRoot) {
      errors.push('未找到有效的插件或技能结构 — 请确保压缩包包含正确的文件结构');
      errors.push('结构 A (标准插件): my-plugin/.claude-plugin/plugin.json + my-plugin/skills/...');
      errors.push('结构 B (技能集合包): my-skills/skills/<skill-name>/SKILL.md');
      errors.push('结构 C (单技能平铺): my-skill/SKILL.md（带 YAML frontmatter）');
      return { passed: false, errors, warnings, summary };
    }
